...
export default function BlogsClient({
  initialPosts = [],
  initialSidebarArticles = [],
  initialAuthors = [],
}: {
  initialPosts?: BlogPost[];
  initialSidebarArticles?: SidebarArticle[];
  initialAuthors?: BlogAuthor[];
}) {
  const [posts, setPosts] = useState<BlogPost[]>(initialPosts);
  const [sidebarArticles, setSidebarArticles] = useState<SidebarArticle[]>(initialSidebarArticles);
    ...
    async function loadBlogs() {
      try {
        ...
        if (data.success && data.posts) {
          setPosts(data.posts);
          if (data.sidebarArticles) setSidebarArticles(data.sidebarArticles);
  ...
  const featuredPost = posts.find((b) => b.featured) || posts[0];

  const gridPosts = posts
    .filter((b) => (featuredPost ? b.id !== featuredPost.id : true))
    .filter((post) => {
    ...
    <main className="min-h-screen bg-[#FAFBFC] text-[#0F172A] relative font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <div className="flex items-center space-x-2 text-[#00796B] font-extrabold text-sm tracking-wide">
          ...
          <span>এই সপ্তাহের বিশেষ ডাইনামিক প্রকাশনা</span>
        </div>

        {loading ? (
          <div className="min-h-[350px] flex items-center justify-center bg-white rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex items-center space-x-3 text-[#00796B] font-bold">
              <Loader2 className="w-6 h-6 animate-spin" />
              <span>ইসলামি ব্লগ পোস্টসমূহ লোড হচ্ছে...</span>
            </div>
          </div>
        ) : (
          <>
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-full text-xs sm:text-sm font-bold transition-all ${
                      selectedCategory === cat
                        ? "bg-[#00796B] text-[#FFFFFF] shadow-md shadow-[#00796B]/20 scale-105"
                      ...
                      >
                        ...
                        <div className="p-5 sm:p-6 pt-0 border-t border-slate-100 flex items-center space-x-3 mt-2">
                          <div className="w-7 h-7 rounded-full bg-[#00796B] text-white font-bold flex items-center justify-center text-xs flex-shrink-0">
                            {b.authorAvatar || b.author?.charAt(0)}
                          </div>
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            <div className="w-9 h-9 rounded-full bg-[#00796B] text-white font-bold flex items-center justify-center text-sm shadow-xs">
                              {author.initial || author.name.charAt(0)}
                            </div>
                            <div>
